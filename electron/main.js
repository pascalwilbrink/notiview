...
function createWindow() {
  ...
  tray.setToolTip('Notiview')
  tray.addListener('click', (evt) => {
    console.log('Tray clicked', evt);
    if (app) {
        app.focus();
    }
  })
  ...
  const startUrl = isDev 
    ? 'http://localhost:4200' 
    : `file://${path.join(__dirname, '../dist/notiview/browser/index.html')}`;
...
app.whenReady().then(createWindow).then(showNotification).catch((e) => {
    console.error(e);
});
